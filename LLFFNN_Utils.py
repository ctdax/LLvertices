...
def fetchSampledEvents(root, signalcsv, bkgcsv, printOutput=True):
    ...
    for filename in dfSubset['Filename']:
        f1 = filename.split('Vtx ')[-1]
        vertex_index = int(f1.split('.png')[0])
        if vertex_index == 0:
            nEvents+=1
...
def significance2(model, data_test, label_test, test_filenames, lumi_data_filepath, signalROOT='all', min_background=50,
                  plotBackground=True, outputFilepath=None, printSampleData=True):
    '''
    ...
    :param printSampleData: If True, prints out the sample and lumi scale data along with the histogram
    ...
    for sr in sroot:
        ...
        if outputFilepath != None:
            ...
            plt.show()
        if printSampleData:
            print('Total signal vertices in histogram: {}\nTotal background vertices in histogram: {}'
                  .format(df_pre_cut[sr].sum(), df_pre_cut['Background Vertex Count'].sum()))
