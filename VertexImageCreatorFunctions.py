...
def drawLine(shape, img, tkdxy_err, max_dxy_err):
    '''
    :param shape: [(x0, y0), (x1, y1)] Coordinates for image creation
    :param img: PIL image
    :param tkdxy_err: Error in tkdxy variable
    :param max_dxy_err: Maximum dxy error
    :return: Draws a line on the image whose magnitude is the magnitude of px and py
    '''
...
def drawTriangle(lineshape, img, tkdxy_err, max_dxy_err, tk_pt, max_tk_pt):
    '''
    :param lineshape: [(x0, y0), (x1, y1)] Coordinates for image creation
    :param img: PIL image
    :param tkdxy_err: Error in tkdxy variable
    :param max_dxy_err: Maximum dxy error
    :param tk_pt: Tk pt
    :param max_tk_pt: Maximum tk pt
    :return: Draw isoceles triangle where magnitude of the track is the height
    '''
...
def ObtainData(stree, btree, beamspot_coordinates, IsSignal=True):
    '''
    :param stree: Signal ROOT tree
    :param btree: Background ROOT tree
    :param beamspot_coordinates: [X, Y, Z] coordinates of the beamspot
    :param IsSignal: True if the data is signal
    :return: px, py, pt, dxy, dxy_err, vtx_x, and vtx_y variables
    '''
    if str(type(stree)) == "<class 'awkward.highlevel.Array'>":
        if IsSignal == True:
            ...
            print("Error: Dataset must be True for signal or False for background", flush=True)
            sys.exit()
    else:
        if IsSignal == True:
...
def calculateAngle(x, y):
    ...
    :param x: X vector
    :param y: Y vector
    :return: Returns the 360 degree angle for a given 2D vector
    '''
    try:
...
def GetMaxVar(var):
    ...
    :return: Returns the maximum value of a particular variable
    '''
    maxL = []
    for a in var:
        for b in a:
...
def FixLogSign(result, log):
    '''
    :param result: Vector magnitude
    :param log: Log of the vector magnitude
    :return: Fixes the sign of the log vector magnitude to match the normal vector magnitude
    '''
    result_fixed = None
    if log > 0:
        if result < 0:
...
def calculateXYImage(px, py, maxpx, maxpy, tk2_dx, tk2_dy, imagecenter, Pscale_multiplicity_constant, DXscale_multiplicity_constant):
    '''
    :param px: px
    :param py: py
    :param maxpx: maxpx
    :param maxpy: maxpy
    :param tk2_dx: Secondary track displacement in x
    :param tk2_dy: Secondary track displacement in y
    :param imagecenter: Center of the image
    :param Pscale_multiplicity_constant: Momentum scale multiplicity constant for rescaling
    :param DXscale_multiplicity_constant: Displacement scale multiplicity constant for rescaling
    :return: Returns the initial and final points for later use as the 'shape' variable
    '''
...
def calculateSecondaryDXY(tk_px, tk_py, tk_dxy, vtx_x, vtx_y):
    '''
    :param tk_px: track px
    :param tk_py: track py
    :param tk_dxy: track displacement
    :param vtx_x: vtx x coordinate
    :param vtx_y: vtx y coordinate
    :return: track dxy with respect to the secondary vertex
    '''
...
def createVertexImage(stree, btree, beamspot_coordinates, event, vertex, w=256, h=256, rootFileName=None, ImageFilePath=None,
        ...
        if counter.value%50 == 0:
            ...
            if IsSignal:
                print('{}/{} Signal Images have been created. Elapsed time for this ROOT: {}. Approximate time remaining for this ROOT: {}'.format(
                        counter.value, Np, elapsed_time, approx_time_remaining), flush=True)
            else:
...
def Observe_N_Vertices(N, stree=None, btree=None, beamspot_coordinates=None, w=256, h=256, IsSignal=True, ImageFilePath=None,
    ...
    if IsSignal:
        ...
        if parallel: #Run function in parallel
            ...
            approx_time_coefficient = (50 + start) / N
            ...
            for i in range(len(event)):
                createVertexImage(stree, btree, beamspot_coordinates, event[i], vertex[i], w=w, h=h, ImageFilePath=ImageFilePath,
                ...
                if n%50 == 0:
                    ...
                    print(
                        '{}/{} Signal Images have been created. Elapsed time for this ROOT: {}. Approximate time remaining for this ROOT: {}'.format(
                            n, N, elapsed_time, approx_time_remaining), flush=True)
                    approx_time_coefficient += (50 / N)
...
def ObtainEventToVertexIterables(track_data, n_vertices='all', start=0): #Takes a root array {stree['vtx_tk_px'].array()} and creates two lists, one for the event number and one for the vertex number for all vertices
    ...
    if start != 0: #Start the dataset at a particular vertex
        ...
        print("Total number of events = {}".format(max(event_l)))
        print("Event index = {}".format(event_index))
        del event_l[:event_index]
        ...
        if len(event_l) == 0:
            print("WARNING: The 'start' variable passed into ObtainEventToVertexIterables was too large. This resulted in an empty event list."
                " The script will now be killed and it is recommended that you reduce 'start'.", flush=True)
            sys.exit()
...
def CountTrackQuadrants(stree, btree, beamspot_coordinates, w=256, IsSignal=True):
    ...
    for event in range(len(px)):
        ...
        if event_number%100 == 0:
            print("The track quadrants up to event {} have been counted for {}. {}/{} of the way done.".format(event_number, data, event_number, total_events), flush=True)
        event_number+=1
                ...
                for track in range(len(spx[event][vertex])):
                    ...
                    if printInformation == True:
                        if track_counter % 100:
                            print("Tk Theta Expected = {}".format(calculateAngle(spx[event][vertex][track], spy[event][vertex][track])), flush=True)
                            print("Tk Theta Actual = {}".format(s_ptheta[track_counter]), flush=True)
                            print("Vtx Theta Expected = {}".format(calculateAngle(svtx_x[event][vertex], svtx_y[event][vertex])), flush=True)
                            print("Vtx Theta Actual = {}".format(s_vtx_theta[vertex_counter]), flush=True)
                            print("Rotated Theta = {}".format(s_ptheta[track_counter] + s_vtx_theta[vertex_counter] + rotation_constant), flush=True)
                    track_counter+=1
                vertex_counter+=1
